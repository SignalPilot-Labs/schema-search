...
class SQLAlchemyExtractor(BaseExtractor):
    ...
    def _extract_columns(self, columns: List[ReflectedColumn]) -> List[ColumnInfo]:
        return [
            {
                "name": col["name"],
                "type": str(col["type"]),
    ...
    def _extract_foreign_keys(
        self, foreign_keys: List[ReflectedForeignKeyConstraint], default_schema: str
    ) -> List[ForeignKeyInfo]:
        return [
            {
    ...
    def _extract_indices(self, indices: List[ReflectedIndex]) -> List[IndexInfo]:
        return [
            {
    ...
    def _extract_constraints(
        self, constraints: List[ReflectedUniqueConstraint]
    ) -> List[ConstraintInfo]:
        return [
            {
    ...
    def _extract_check_constraints(
        self, constraints: List[ReflectedCheckConstraint]
